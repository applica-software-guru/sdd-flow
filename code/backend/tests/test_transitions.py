...
async def test_cannot_transition_closed_bug(client: AsyncClient, test_tenant, test_project):
    resp = await client.post(
        ...
        json={"title": "Closed bug", "body": "body", "severity": "minor"},
    )
    bug_id = resp.json()["id"]

    # Close it
    await client.post(
        f"{_bugs(test_tenant, test_project)}/{bug_id}/transition", json={"status": "closed"}
    )

    # Attempt to re-transition a closed bug
    resp = await client.post(
        f"{_bugs(test_tenant, test_project)}/{bug_id}/transition", json={"status": "open"}
    )
...
async def test_cannot_transition_deleted_bug(client: AsyncClient, test_tenant, test_project):
    resp = await client.post(
        _bugs(test_tenant, test_project),
        json={"title": "Deleted bug", "body": "body", "severity": "minor"},
    )
    bug_id = resp.json()["id"]

    await client.post(
        f"{_bugs(test_tenant, test_project)}/{bug_id}/transition", json={"status": "deleted"}
    )
...
async def test_bug_valid_transitions(client: AsyncClient, test_tenant, test_project):
    resp = await client.post(
        _bugs(test_tenant, test_project),
        json={"title": "Transition test", "body": "body", "severity": "major"},
    )
...
async def test_resolved_bug_sets_closed_at(client: AsyncClient, test_tenant, test_project):
    resp = await client.post(
        _bugs(test_tenant, test_project),
        json={"title": "Resolved bug", "body": "body", "severity": "trivial"},
    )
    bug_id = resp.json()["id"]

    resp = await client.post(
        f"{_bugs(test_tenant, test_project)}/{bug_id}/transition", json={"status": "resolved"}
    )
...
async def test_cannot_transition_closed_cr(client: AsyncClient, test_tenant, test_project):
    resp = await client.post(
        _crs(test_tenant, test_project), json={"title": "Closed CR", "body": "body"}
    )
    cr_id = resp.json()["id"]

    await client.post(
        f"{_crs(test_tenant, test_project)}/{cr_id}/transition", json={"status": "closed"}
    )

    resp = await client.post(
        f"{_crs(test_tenant, test_project)}/{cr_id}/transition", json={"status": "pending"}
    )
    assert resp.status_code == 409
...
async def test_cannot_transition_deleted_cr(client: AsyncClient, test_tenant, test_project):
    resp = await client.post(
        _crs(test_tenant, test_project), json={"title": "Deleted CR", "body": "body"}
    )
    cr_id = resp.json()["id"]

    await client.post(
        f"{_crs(test_tenant, test_project)}/{cr_id}/transition", json={"status": "deleted"}
    )
...
async def test_cr_applied_sets_closed_at(client: AsyncClient, test_tenant, test_project):
    resp = await client.post(
        _crs(test_tenant, test_project), json={"title": "Applied CR", "body": "body"}
    )
    cr_id = resp.json()["id"]

    resp = await client.post(
        f"{_crs(test_tenant, test_project)}/{cr_id}/transition", json={"status": "applied"}
    )
    assert resp.status_code == 200
    assert resp.json()["closed_at"] is not None
...
async def test_cr_formatted_number(client: AsyncClient, test_tenant, test_project):
    resp = await client.post(
        _crs(test_tenant, test_project), json={"title": "Numbered CR", "body": "body"}
    )
    assert resp.status_code == 201
    data = resp.json()
