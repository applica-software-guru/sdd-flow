...
async def test_list_bugs_pagination(client: AsyncClient, test_tenant, test_project):
    ...
    for i in range(5):
        await client.post(
            ...
            json={"title": f"Pagination bug {i}", "body": "body", "severity": "minor"},
        )
...
async def test_list_crs_pagination(client: AsyncClient, test_tenant, test_project):
    for i in range(4):
        await client.post(
            _crs(test_tenant, test_project), json={"title": f"Pagination CR {i}", "body": "body"}
        )
...
async def test_list_crs_sorted_by_number_descending(client: AsyncClient, test_tenant, test_project):
    ...
    for i in range(3):
        await client.post(
            _crs(test_tenant, test_project), json={"title": f"Sorting CR {i}", "body": "body"}
        )

    resp = await client.get(_crs(test_tenant, test_project))
    assert resp.status_code == 200
    numbers = [item["number"] for item in resp.json()["items"]]
    assert numbers == sorted(numbers, reverse=True), f"CR list not sorted by number desc: {numbers}"


@pytest.mark.asyncio
async def test_list_bugs_sorted_by_number_descending(
    ...
    for i in range(3):
        await client.post(
            _bugs(test_tenant, test_project),
            json={"title": f"Sorting Bug {i}", "body": "body", "severity": "minor"},
        )
...
async def test_list_crs_first_item_is_latest(client: AsyncClient, test_tenant, test_project):
    ...
    await client.post(_crs(test_tenant, test_project), json={"title": "First", "body": "b"})
    await client.post(_crs(test_tenant, test_project), json={"title": "Second", "body": "b"})
...
async def test_list_bugs_first_item_is_latest(client: AsyncClient, test_tenant, test_project):
    """The first item returned must be the one with the highest progressive number."""
    await client.post(
        _bugs(test_tenant, test_project), json={"title": "Bug A", "body": "b", "severity": "minor"}
    )
    await client.post(
        _bugs(test_tenant, test_project), json={"title": "Bug B", "body": "b", "severity": "minor"}
    )
    r3 = await client.post(
        _bugs(test_tenant, test_project), json={"title": "Bug C", "body": "b", "severity": "minor"}
    )
...
async def test_list_crs_sort_stable_across_pages(client: AsyncClient, test_tenant, test_project):
    ...
    for i in range(5):
        await client.post(
            _crs(test_tenant, test_project), json={"title": f"Page CR {i}", "body": "b"}
        )
...
async def test_list_bugs_sort_stable_across_pages(client: AsyncClient, test_tenant, test_project):
    ...
    for i in range(5):
        await client.post(
            _bugs(test_tenant, test_project),
            json={"title": f"Page Bug {i}", "body": "b", "severity": "minor"},
        )
...
async def test_status_filter_excludes_deleted(client: AsyncClient, test_tenant, test_project):
    resp = await client.post(
        _bugs(test_tenant, test_project),
        json={"title": "Soon deleted", "body": "body", "severity": "minor"},
    )
    bug_id = resp.json()["id"]
    await client.post(
