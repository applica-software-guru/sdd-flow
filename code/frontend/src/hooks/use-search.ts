...
export function useSearch(tenantId: string | undefined, query: string, type?: SearchTypeFilter) {
  return useQuery<SearchResult[]>({
    queryKey: queryKeys.search.list(tenantId, query, type),
    queryFn: async () => {
      const params = new URLSearchParams({ q: query });
      if (type) params.set('type', type);
      const { data } = await api.get(`/tenants/${tenantId}/search?${params}`);
      return (data.results ?? data).map((r: Record<string, unknown>) => ({
        ...
        url: buildUrl(requireRouteParam(tenantId, 'tenantId'), r),
      }));
    },
