...
const LAST_TENANT_STORAGE_KEY = 'sdd-last-tenant-id';
...
function writeLastTenantId(tenantId: string): void {
  try {
    localStorage.setItem(LAST_TENANT_STORAGE_KEY, tenantId);
  } catch {
    // Storage unavailable (e.g. private mode) — fallback stays in memory
  }
}
...
export default function Layout() {
  ...
  // Remember the last visited tenant so the sidebar keeps its content on
  // tenant-independent pages (e.g. /settings/profile) instead of going empty.
  // Render-time state adjustment (same pattern as AssignmentPanel — see
  // "You Might Not Need an Effect"): when the URL tenant changes we update
  // the remembered value immediately, no effect needed.
  const [lastTenantId, setLastTenantId] = useState<string | null>(readLastTenantId);
  if (tenantId && tenantId !== lastTenantId) {
    setLastTenantId(tenantId);
    writeLastTenantId(tenantId);
  }

  // Tenant context for the sidebar/menu: the URL tenant when present,
  // otherwise the last visited tenant.
  const sidebarTenantId = tenantId ?? lastTenantId ?? undefined;
  ...
  const projectSection = useMemo(() => {
    ...
    if (trailingPath.startsWith('/workers')) {
      return 'Workers';
    }
        ...
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="fixed inset-y-0 left-0 w-64 bg-white shadow-xl dark:bg-slate-800">
            ...
            <nav className="flex flex-col gap-1 px-3">
              {sidebarTenantId && (
                <>
                  <NavLink to={`/tenants/${sidebarTenantId}`} end className={navLinkClass}>
                  ...
                  {isAdmin && (
                    <NavLink to={`/tenants/${sidebarTenantId}/settings`} className={navLinkClass}>
                      <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                  ...
                  {isAdmin && (
                    <NavLink to={`/tenants/${sidebarTenantId}/audit-log`} className={navLinkClass}>
                      <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                  ...
                  {projectId && (
                    <>
                      ...
                      <NavLink to={`/tenants/${tenantId}/projects/${projectId}/workers`} className={projectNavLinkClass}>
                        <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
          ...
          <div className="relative" ref={userMenuRef}>
            ...
            {userMenuOpen && (
              <div className="absolute right-0 top-full z-50 mt-1 w-48 rounded-md border border-slate-200 bg-white py-1 shadow-lg dark:border-slate-700 dark:bg-slate-800">
                <div className="min-w-0 border-b border-slate-100 px-4 py-2 dark:border-slate-700">
                  <p className="truncate text-sm font-medium text-slate-900 dark:text-slate-100" title={user?.display_name}>
                    {user?.display_name}
                  </p>
                ...
                <Link
                  ...
                  className="flex w-full items-center gap-2 border-b border-slate-100 px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-700"
                >
                  <svg
                    className="h-4 w-4"
                    fill="none"
          ...
          <nav className="flex flex-col gap-1 p-3">
            {sidebarTenantId && (
              <>
                ...
                {isAdmin && (
                  <NavLink
                    to={`/tenants/${sidebarTenantId}/settings`}
                    className={navLinkClass}
                  >
                    <svg
                      className="h-4 w-4"
                ...
                {isAdmin && (
                  <NavLink
                    to={`/tenants/${sidebarTenantId}/audit-log`}
                    className={navLinkClass}
                  >
                    <svg
                      className="h-4 w-4"
          ...
          {isProjectScoped && tenantId && projectId && (
            <div className="mx-auto mb-5 max-w-5xl space-y-3">
              <div className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
                <NavLink to={`/tenants/${sidebarTenantId}`} className="font-medium text-slate-600 hover:text-slate-900 dark:text-slate-300 dark:hover:text-white">
                  Projects
                </NavLink>
                <span>/</span>
