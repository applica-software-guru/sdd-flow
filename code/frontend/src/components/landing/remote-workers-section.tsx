const steps = [
  {
    ...
    description:
      "One command and you're ready. The CLI runs on your machine, your CI server, or anywhere you keep your codebase.",
  ...
  },
  {
    ...
    description:
      'Point it at your project and let it connect. The worker registers itself and waits silently for jobs to arrive.',
  ...
  },
  {
    ...
    description:
      'Open a Change Request or Bug in SDD Flow, hit "Run on Worker", and watch it get handled — no terminal required.',
    code: null,
  },
];
...
const benefits = [
  {
    ...
    description:
      'The worker runs locally. SDD Flow only sends instructions and receives results — your codebase stays where it is.',
  ...
  },
  {
    ...
    description:
      'Got a CI box and a local machine? Connect both. SDD Flow distributes jobs across all available workers automatically.',
  ...
  },
  {
    ...
    description:
      'Claude, a custom script, or any compatible agent — the worker is model-agnostic. Use what you already have.',
  ...
  },
  {
    ...
    description:
      'Every message the agent writes streams live into SDD Flow. You see exactly what it did, step by step.',
...
export default function RemoteWorkersSection() {
  return (
    <section id="remote-workers" className="scroll-mt-14 px-4 py-20 sm:py-24 lg:py-32">
      <div className="mx-auto max-w-7xl sm:px-6 lg:px-8">
        ...
        <div className="text-center">
          ...
          <p className="mx-auto mt-4 max-w-2xl text-lg text-slate-600 dark:text-slate-400">
            Remote Workers are background agents that run on your infrastructure and pick up jobs
            from SDD Flow automatically. Assign a Change Request, walk away — it's done when you get
            back.
          </p>
        </div>
          ...
          <div className="space-y-8">
            {steps.map((step) => (
              <div key={step.number} className="flex gap-5">
                <div className="flex-shrink-0 select-none font-mono text-3xl font-bold text-slate-200 dark:text-slate-700">
                  {step.number}
                </div>
                <div>
                ...
                <div key={w.name} className="flex items-center justify-between px-5 py-4">
                  <div className="flex items-center gap-3">
                    <div className="relative">
                      ...
                      <span
                        className={`absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 rounded-full border-2 border-white dark:border-slate-800 ${w.status === 'busy' ? 'bg-amber-400' : 'bg-emerald-400'}`}
                      />
                    </div>
                    <div>
        ...
        <div className="mt-16 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {benefits.map((b) => (
            ...
            >
              <div
                className={`mb-3 flex h-10 w-10 items-center justify-center rounded-lg ${b.bg} ${b.color}`}
              >
                {b.icon}
              </div>
